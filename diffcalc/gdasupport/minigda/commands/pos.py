    ...
    def posReturningReport(self, *posargs):
        ...
        if len(posargs) == 1:
            ...
            if not isinstance(scannable, Scannable):
                raise TypeError("The first argument to the pos command must be a scannable")
        ...
        # Move the scannable and report
            ...
            # TODO: minigda assumes all moves complete instantly, so no need yet to check the move is complete
            return self._generatePositionReport(scannable)
            
        else:
            raise ValueError("Invlaid arguements: 'pos [ scannable [ value ] ]'")

    def _generatePositionReport(self, scannable):
        fieldNames = tuple(scannable.getInputNames()) + tuple(scannable.getExtraNames())
        # All scannables
        ...
        # Multi field scannable:
            ...
            for name, formattedValue in zip(fieldNames, scannable.formatPositionFields(pos)):
