...
Starts minigda and if specified runs a script. path_to_script may be absolute or
relative to the directory from which minigda is run."""
...
#    print "\nsys.argv: ",sys.argv
    ...
    if STARTUPSCRIPT is not None:
        if os.path.exists(STARTUPSCRIPT):
            ...
            print "ERROR: No startup script with abosolute or relative path '%s' found." % sys.argv[1]
            usage()
            sys.exit(2)
