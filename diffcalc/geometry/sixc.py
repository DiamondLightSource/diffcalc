...
class SixCircleGammaOnArmGeometry(DiffractometerGeometryPlugin):
    """
    This six-circle diffractometer geometry simply passes through the angles from
    a six circle diffractometer with the same geometry and angle names as those
    defined in Vliegs's paper defined internally.
    """
...
#(deltaB, gammaB) = gammaOnArmToBase(deltaA, gammaA, alpha) (all in radians)
    ...
    def internalPositionToPhysicalAngles(self, internalPosition):
        ...
        if self.hardwareMonitor is not None:
            ...
            # print "SixCircleGeometry applying transform: gammaB -> gammaB - 180, and deltaB -> 180 - deltaB"
...
WARNING: This diffractometer has the gamma circle attached to the base rather than the end of
         the delta arm as Vlieg's paper defines. A conversion has been made from
         the physical angles to their internal representation (gamma-on-base-to-arm). This
         conversion has forced gamma to be positive by applying the mapping:
            delta --> 180+delta 
            gamma --> 180+gamma.
...
def gammaOnBaseToArm(deltaB, gammaB, alpha):
    ...
    Maps delta and gamma for an instrument where the gamma circle rests on the base
    to the case where it is on the delta arm.
    
    There are always two possible solutions. To get the second apply the transform:
    ...
    if solvesEq8(alpha, deltaA1, gammaA1, deltaB, gammaB):
        ...
        print "gammaOnBaseToArm choosing 2nd delta root and 2nd gamma root (to internal)"
...
WARNING: This diffractometer has the gamma circle attached to the base rather than the end of
         the delta arm as Vlieg's paper defines. A conversion has been made from
         the internal representation of angles to physical angles (gamma-on-arm-to-base). This
         conversion has forced gamma to be positive by applying the mapping:
            delta --> 180-delta 
            gamma --> 180+gamma.
         This should have no adverse effect.
...
def gammaOnArmToBase(deltaA, gammaA, alpha):
    ...
    Maps delta and gamma for an instrument where the gamma circle is on the delta
    arm to the case where it rests on the base.
    
    There are always two possible solutions. To get the second apply the transform:
        delta --> 180-delta (reflect and flip top opposite side of circle)
        gamma --> 180+gamma (flip to opposite side of circle)
       
    This code will return the solution where gamma is positive, but will warn if a sign
    change was made.
    """
    ...
    if solvesEq8(alpha, deltaA, gammaA, deltaB1, gammaB1) and 0 <= gammaB1 <= pi:
        ...
        raise RuntimeError("No valid solutions found mapping from gamma-on-arm to gamma-on-base")
