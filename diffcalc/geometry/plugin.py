class DiffractometerGeometryPlugin:
...
# Required methods
    ...
    def __init__(self, name, supportedModeGroupList, fixedParameterDict, gammaLocation, mirrorInXzPlane = False):
        ...
        circle (dictionary). Define wether the gamma angle is on the 'arm' or the 'base'; used
        only by AngleCalculator to interpret the gamma parameter in fixed gamma mode: for 
        instruments with gamma on the base, rather than on the arm as the code assume internally,
        the two methods physicalAnglesToInternalPosition and internalPositionToPhysicalAngles
        must still be used.
        """    
        if gammaLocation not in ('arm', 'base', None):
            raise RuntimeError("Gamma must be on either 'arm' or 'base' or None")
