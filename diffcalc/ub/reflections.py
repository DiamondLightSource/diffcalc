...
class Reflection:
    ...
    def __str__(self):
        return "energy=%-6.3f h=%-4.2f k=%-4.2f l=%-4.2f  alpha=%-8.4f delta=%-8.4f gamma=%-8.4f omega=%-8.4f chi=%-8.4f phi=%-8.4f  %-s %s" % (self.energy, self.h, self.k, self.l, self.pos.alpha, self.pos.delta, self.pos.gamma, self.pos.omega, self.pos.chi, self.pos.phi, self.tag, self.time)            
...
class ReflectionList:
    ...
    def addReflection(self, h, k, l, position, energy, tag, time):
        """addReflection(self, h, k, l, position, tag=None) -- adds a reflection
    ...
    def soStringWithInternalAngles(self):
        toReturn = "      %-6s %-4s %-4s %-4s  %-8s %-8s %-8s %-8s %-8s %-8s  tag\n" % \
        ('energy', 'h', 'k', 'l', 'alpha', 'delta', 'gamma', 'omega', 'chi', 'phi')
        #if len(self._reflist)==0:
        #    toReturn += "   <<empty>>"
        for n in range(len(self._reflist)):
            ref = self._reflist[n]
