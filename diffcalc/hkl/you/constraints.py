...
class ConstraintManager(object):
    ...
    def __init__(self, hardware):
        ...
        self._constrained = {} # dictionary of constrained values, may be
        self._tracking = []
...
###
    ...
    def _label_constraint(self, name):
        if name == '':
            label = '    '      
        elif self.is_tracking(name): # imples constrained
            label = '~~> ' 
...
###
    ...
    def set(self, name, value):
        ...
        if name in self._tracking:
            raise DiffcalcException(
"""Could not set %s as this constraint is configured to track its associated
physical angle. First remove this tracking (use 'untrack %s').""" % (name, name))
